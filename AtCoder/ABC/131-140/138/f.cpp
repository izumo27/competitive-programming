...
Mint solve(){
  ...
  dp[60][0][0][0]=1;
  bool ok=true;
  FORR(i, 0, n+1) REP(x, 2) REP(y, 2) REP(z, 2){
    int bl=l[i], br=r[i];
    Mint t=dp[i+1][x][y][z];
    // 1,1
    dp[i][f(x, bl, 1)][g(y, br, 1)][1]+=t;
    // 0,0
    dp[i][f(x, bl, 0)][g(y, br, 0)][z]+=t;
    // 0,1
    if(z) dp[i][f(x, bl, 0)][g(y, br, 1)][z]+=t;
  }
  Mint res=0;
