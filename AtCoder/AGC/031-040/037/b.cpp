...
vi num[3];
int ord[3][125252];
int mi[125252], mid[125252], ma[125252];
int n;
string s;

vl f(int x){
  int idx=1, l=0, crt=ord[x][0];
  vl res;
  while(idx<n){
    if(crt!=ord[x][idx]){
      crt=ord[x][idx];
      ...
      if(crt==0){
        while(l<idx && mid[l]<num[x][idx]){
          ++l;
        }
      }
      else if(crt==1){
        while(l<idx && (num[x][l]<mi[idx] || num[x][idx]>ma[l])){
          ++l;
        }
      }
      else{
        while(l<idx && mid[idx]>num[x][l]){
          ++l;
        }
      }
    }
    res.push_back(idx-l+1);
    ++idx;
  }
  return res;
}
...
int main(){
  ...
  REP(i, n){
    if(num[0][i]<num[1][i]){
      ord[0][i]=(num[0][i]<num[2][i] ? 0 : 1);
    }
    else{
      ord[0][i]=(num[0][i]>num[2][i] ? 2 : 1);
    }
    if(num[1][i]<num[0][i]){
      ord[1][i]=(num[1][i]<num[2][i] ? 0 : 1);
    }
    else{
      ord[1][i]=(num[1][i]>num[2][i] ? 2 : 1);
    }
    if(num[2][i]<num[0][i]){
      ord[2][i]=(num[2][i]<num[1][i] ? 0 : 1);
    }
    else{
      ord[2][i]=(num[2][i]>num[1][i] ? 2 : 1);
    }
  }
  vl ans1=f(0);
  vl ans2=f(1);
  ...
  Mint ans=1;
  REP(i, n-1){
    ans*=ans1[i]*ans2[i]*ans3[i]/min({ans1[i], ans2[i], ans3[i]});
  }
  Comb comb(n);
  ans*=comb.fact(n);
  cout<<ans<<'\n';
