...
template<typename T> Mint kitamasa(int k, Mint *a, Mint *d, T n){
	Mint b[k];
	b[0]=1;
	T mask=1, m=n;
	while(m>1){
	...
	while(mask>0){
		...
		FOR(i, 1, k){
			c[i][0]=0;
			FOR(j, 1, k) c[i][j]=c[i-1][j-1];
			REP(j, k) c[i][j]+=c[i-1][k-1]*d[j];
		}
		REP(i, k) b[i]=0;
		REP(i, k)REP(j, k) b[j]+=c[0][i]*c[i][j];
		if(n&mask){
			// n -> n+1
			rotate(b, b+(k-1), b+k);
			Mint t=b[0];
